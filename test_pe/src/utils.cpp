...
void ZeroInitF32(float *p_data, int N) {
    int k;
    for (k = 0; k < N; k++) {
        p_data[k] = 0;
    }
...
/*void ZeroInitF32(uchar *p_data, int N) {
    int k;
    for (k = 0; k < N; k++) {
        p_data[k] = 0;
    }
...
bool VectorCompare(const float *t1, const float *t2, int dim) {
  bool ret = true;
  float mse = 0;
  float tmp = 0 ;
  ...
  for (int n = 0; n < dim; ++n) {
    tmp = pow(t1[n] - t2[n], 2);
    if(( tmp > 1e-6 )&&(total_error<100000)) {
      total_error = total_error + 1 ;
      printf("data1 = %f.data2 = %f.mse = %f.index=%d\n", t1[n],t2[n],tmp,n);
    }
  }
  mse = mse / dim;
  if (mse > 1e-6) {
