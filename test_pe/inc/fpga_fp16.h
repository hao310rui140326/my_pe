...
STATIC half_t float_to_half(float orig_float)
{
  ...
  if (exponent <= FP32_EXPONENT_BIAS) {
#if defined(DISABLE_SUBNORM_FLOATS)
    mantissa = 0;
#else
    switch (exponent) {
      case (FP32_EXPONENT_BIAS-0) : mantissa = (1 << 12) | (BIT_SEL(mantissa, 22, 12) << 1) | (BIT_SEL(mantissa, 11, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-1) : mantissa = (1 << 11) | (BIT_SEL(mantissa, 22, 13) << 1) | (BIT_SEL(mantissa, 12, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-2) : mantissa = (1 << 10) | (BIT_SEL(mantissa, 22, 14) << 1) | (BIT_SEL(mantissa, 13, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-3) : mantissa = (1 <<  9) | (BIT_SEL(mantissa, 22, 15) << 1) | (BIT_SEL(mantissa, 14, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-4) : mantissa = (1 <<  8) | (BIT_SEL(mantissa, 22, 16) << 1) | (BIT_SEL(mantissa, 15, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-5) : mantissa = (1 <<  7) | (BIT_SEL(mantissa, 22, 17) << 1) | (BIT_SEL(mantissa, 16, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-6) : mantissa = (1 <<  6) | (BIT_SEL(mantissa, 22, 18) << 1) | (BIT_SEL(mantissa, 17, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-7) : mantissa = (1 <<  5) | (BIT_SEL(mantissa, 22, 19) << 1) | (BIT_SEL(mantissa, 18, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-8) : mantissa = (1 <<  4) | (BIT_SEL(mantissa, 22, 20) << 1) | (BIT_SEL(mantissa, 19, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-9) : mantissa = (1 <<  3) | (BIT_SEL(mantissa, 22, 21) << 1) | (BIT_SEL(mantissa, 20, 0) != 0); break;
      case (FP32_EXPONENT_BIAS-10): mantissa = (1 <<  2) | (BIT_SEL(mantissa, 22, 22) << 1) | (BIT_SEL(mantissa, 21, 0) != 0); break;
      default:  mantissa = 0; break;
   } 
#endif
    exponent = 0;
  } else if (exponent > (FP32_EXPONENT_BIAS + 0x01e)) {
    // exponent is too large for half precision, return infinity
    exponent = 0x1f;
    mantissa = 0;
  } else {
    // normal situation, translate the value
    exponent = exponent - FP32_EXPONENT_BIAS;
